...
FILE *fpin,*fpout;
bool IsT2;
long long RecNum, revNum;
long long oflcorrection;
long long truensync, truetime, prevtrigtime;
int m, c;
double GlobRes = 0.0;
...
bool skipped_initial_nonzero = false;
...
void GotPhoton(long long TimeTag, int Channel, int DTime)
{
  if(IsT2)
  {
      if (!skipped_initial_nonzero){
        if (Channel == 0){
          skipped_initial_nonzero = true;
          prevtrigtime = TimeTag;
          revNum = 0;
          fprintf(fpout,"revolution,channel,delay\n");
        }
      }
      else
      {
        ...
        {
          fprintf(fpout,"%I64u, %1x, %I16u\n", revNum, Channel, TimeTag-prevtrigtime);
        }
      }
  }
  else
...
// End Header loading
  ...
  switch (RecordType)
  {
    ...
    case rtHydraHarpT3:
      ...
      // fprintf(fpout,"\nrecord# chan   nsync truetime/ns dtime\n");
      break;
    case rtHydraHarp2T2: //this is our case
